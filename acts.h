...
extern char OnWalkOver(ENTITY* Walker, ENTITY* WalkedOver)
{
  ...
  if (HasDataFlag(WalkedOver, FLAG_CHANGE_SPEED_ON_STEP) && HasDataFlag(Walker, FLAG_SPEED))
  {
    int* SC;
    GetDataFlag(WalkedOver, FLAG_CHANGE_SPEED_ON_STEP, (void**)&SC);
    int* S;
    GetDataFlag(Walker, FLAG_SPEED, (void**)&S);
    *S += *SC;
    if (*S < 0) *S = 0;
  }
  return 0;
