 ...
 * @return The index
 */
extern unsigned long b_getIndex(const int row, const int col)
{
  ...
  for (int row = 0; row < S_ROW; row++)
  {
    ...
    for (int col = 0; col < S_COL; col++)
    {
        ...
        || lastEffect != pixel->effect)
      {
        ...
        if (lastBackColor)
          sprintf(sBuffer + strlen(sBuffer), "\033[48;5;%dm", lastBackColor);
        sprintf(sBuffer + strlen(sBuffer), "\033[38;5;%dm", lastColor);
      }
...
extern unsigned char* b_getOccluded(const ENTITY* map, const ENTITY* player)
{
  ...
  for (int oX = 0; oX < MAP_WIDTH; oX++)
  {
    for (int oY = 0; oY < MAP_HEIGHT; oY++)
    {
      const int dX = oX - x;
      const int dY = oY - y;
      if (dX * dX + dY * dY > ((float)(*ViewRange * *ViewRange) - 0.05))
      {
        occluded[oX + oY * S_COL] |= 0b100;
        continue;
      }
      if (occluded[oX + oY * S_COL] & 0b10)
        continue;
      int count;
      int** line = createLine(dX, dY, &count);
...
extern void b_writeMapToBuffer(B_BUFFER* buffer, const ENTITY* map, const ENTITY* player)
{
  ...
  for (int i = 0; i < MAP_LENGTH; i++)
    drawn[i] = 0;
  ENTITY** ELIST;
  unsigned char* occluded = nullptr;
  if (player)
  ...
  for (int i = 0; ELIST[i]; i++)
  {
    ...
    if (player && occluded[x + y * S_COL] & 0b110)
    {
      continue;
    }
  ...
  if (player)
  {
    for (int x = 0; x < S_COL; x++)
    {
      ...
      {
        if (occluded[x + y * S_COL] & 0b100)
          b_setPixel(buffer, y + 2, x, &PIXEL_VISION_OUT_OF_RANGE);
        else if (occluded[x + y * S_COL] & 0b10)
...
extern void b_writeHudToBuffer(B_BUFFER* buffer, const ENTITY* player)
{
  ...
  if (HP)
  {
    ...
    b_writeToColor(buffer, S_ROW - 1, xCur + 1, HPText, color);
    xCur += 3 + HPLen * 2 + 4;

    free(HPText);
  }

  char* SpeedText = calloc(MAX_STR_LEN, sizeof(char));
  ...
  b_writeTo(buffer, S_ROW - 1, xCur + 1, "MT: ");
  b_writeToColor(buffer, S_ROW - 1, xCur + 5, SpeedText, color);
  free(SpeedText);
