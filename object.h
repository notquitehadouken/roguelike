...
extern ENTITY* EntityLookup(const int uid)
{
...
/**
 * The amount of time passed, counted in 1/100 second increments
 */
 ...
 */
int ScaleTime(int Time, const ENTITY* E)
{
  if (Time == 0 || !E)
    return Time;
  int* s;
 ...
 */
int GTimeAdvance(const int Time, ENTITY* E)
{
  if (!Time)
    return 0;
  GLOBAL_TIMER += Time;
